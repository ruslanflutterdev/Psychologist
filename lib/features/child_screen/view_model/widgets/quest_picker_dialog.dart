...
class QuestPickerDialog extends StatefulWidget {
  ...
  final List<String> takenQuestIds;

  const QuestPickerDialog({
    super.key,
    required this.catalog,
    required this.takenQuestIds,
  });
...
class _QuestPickerDialogState extends State<QuestPickerDialog> {
  ...
  Widget build(BuildContext context) {
    final availableQuests = _all
        .where((q) => q.active && !widget.takenQuestIds.contains(q.id))
        .toList();
    final filtered = _filter == null
        ? availableQuests
        : availableQuests.where((q) => q.type == _filter).toList();
