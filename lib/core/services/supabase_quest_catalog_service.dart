...
String _sphereFromType(QuestType t) {
  switch (t) {
    case QuestType.physical:   return 'physical';
    case QuestType.emotional:  return 'emotional';
    case QuestType.cognitive:  return 'cognitive';
    case QuestType.social:     return 'social';
    case QuestType.spiritual:  return 'spiritual';
  }
}

QuestType _typeFromSphere(String? sphere) {
  switch ((sphere ?? 'general').toLowerCase()) {
    case 'physical':   return QuestType.physical;
    case 'emotional':  return QuestType.emotional;
    case 'cognitive':  return QuestType.cognitive;
    case 'social':     return QuestType.social;
    case 'spiritual':  return QuestType.spiritual;
    default:           return QuestType.physical;
  }
}
...
class SupabaseQuestCatalogService implements QuestCatalogService {
  ...
  Quest _mapRow(Map<String, dynamic> r) {
    return Quest(
      ...
      updatedAt: DateTime.tryParse((r['updated_at'] ?? r['created_at']).toString()) ?? DateTime.now(),
    );
  }

  List<String> get _columns => const [
    'id', 'title', 'description', 'created_id', 'active',
    'created_at', 'updated_at', 'xp', 'sphere'
  ];
  ...
  Stream<List<Quest>> getQuests({required QuestCatalogFilter filter}) {
    ...
    return stream.map((rows) {
      final list = rows.map<Map<String, dynamic>>((e) => e).map(_mapRow).toList();
      var filtered = list;
