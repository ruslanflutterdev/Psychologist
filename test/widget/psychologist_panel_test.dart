...
// --- TEST DATA ---
final testProfile =
const PsychologistModel(firstName: 'Тест', lastName: 'Психолог');
  ...
  group('PsychologistBody Data Loading and Realtime', () {
    ...
    testWidgets('Проверка обработки ошибки загрузки профиля', (tester) async {
      ...
      when(() => mockPsychologistService.getProfile()).thenAnswer(
              (_) => Future.error(Exception('Профиль не найден')));

      when(() => mockChildService.getChildren())
          .thenAnswer((_) => Stream.value(initialChildren));

      await tester.pumpWidget(createWidgetUnderTest(childStream: Stream.value(initialChildren)));
      await tester.pumpAndSettle();
      ...
      expect(find.byType(ChildTile), findsNWidgets(2), reason: 'Список детей должен загрузиться, несмотря на ошибку профиля');
    });
    ...
    testWidgets('Проверка обработки ошибки Stream для детей', (tester) async {
      ...
      expect(find.textContaining('Ошибка загрузки: Ошибка Realtime'), findsOneWidget);
